...
public interface OrderRepository extends JpaRepository<OrderEntity, Long> {
	...
	@Query(value = "SELECT * from orders  WHERE  (project_id = :projectId) AND (:adminCheckInId is null OR admin_checkin_id = :adminCheckInId)  AND (:cardCode is null OR card_code = :cardCode)  AND  (:cardStt is null OR card_stt = :cardStt)  AND (:carNumber is null OR car_number like %:carNumber%) AND ((checkout_time >= :dateFrom AND checkout_time <= :dateTo) OR ((checkin_time >= :dateFrom AND checkin_time <= :dateTo) AND (checkout_time = 0 OR checkout_time > :dateTo))) order by updated DESC", 
			countQuery = "SELECT * from orders  WHERE  (project_id = :projectId) AND (:adminCheckInId is null OR admin_checkin_id = :adminCheckInId) AND (:cardCode is null OR card_code = :cardCode)  AND  (:cardStt is null OR card_stt = :cardStt)  AND (:carNumber is null OR car_number like %:carNumber%) AND ((checkout_time >= :dateFrom AND checkout_time <= :dateTo) OR ((checkin_time >= :dateFrom AND checkin_time <= :dateTo) AND (checkout_time = 0 OR checkout_time > :dateTo))) order by updated DESC", nativeQuery = true)
	...
	@Query(value = "SELECT * from orders  WHERE  project_id = :projectId AND vehicle_id = :vehicle_id AND (checkout_time =0 || checkout_time is null ) order by updated DESC", nativeQuery = true)
	List<OrderEntity> findAllVehicleExistingInParking(@Param(value = "projectId") String projectId,@Param(value = "vehicle_id") int vehicleId);
}
