...
public class OrderServiceImpl implements OrderService {
	...
	public OrderDto findByOrderId(Long orderId) {
		OrderEntity entities = orderRepository.findByOrderId(orderId);
		if (entities == null) {
			return null;
		}
		OrderDto dto = new OrderDto();
	...
	public ResultObject<List<OrderDto>> findAll(Pageable pageable, OrderSearchForm orderSearchForm) {
		...
		if(orderSearchForm.getIsMonthlyCard() == 0) {
			orderRepository.search(orderSearchForm.getProjectId(), orderSearchForm.getCardCode(), orderSearchForm.getCardStt(), 
